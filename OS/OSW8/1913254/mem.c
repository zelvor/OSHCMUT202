...
void * best_fit_allocator(unsigned int size) {
	...
	int found = 0;
	unsigned int best_spacing = __UINT32_MAX__;
  	struct mem_region *current_region = free_regions, *best_region = free_regions;
  	do
  	{
    if (size <= current_region->size)
    {
      int diff = current_region->size - size;
      if (diff < best_spacing)
      {
        best_spacing = diff;
    ...
    current_region = current_region->next;
  	} while (found != 2 && current_region != NULL);
